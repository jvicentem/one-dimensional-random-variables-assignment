...
Dicho teorema dice que para cualquier población con media $\mu$, desviación estándar $\sigma$, y *k* > 1, la probabilidad de que las observaciones estén dentro del intervalo $\left [ \mu \pm k\sigma  \right ]$ es de al menos $1 - \frac{1}{k^2}$, donde k es el número de desviaciones estándar.
...
Descartamos el valor negativo ya que *k* debe ser mayor que 1, por lo que $k = \sqrt{\frac{1}{0.5}} = \sqrt{2}$.

Calculamos $\left [ \mu \pm k\sigma  \right ]$ ahora que sabemos el valor de *k*:

$$ \left [ \frac{35}{3} \pm \sqrt{2}\sqrt{\frac{200}{9}}  \right ] $$

```{r, warning=FALSE}
(35/3)-(sqrt(2)*sqrt(200/9))
(35/3)+(sqrt(2)*sqrt(200/9))
```
...
Lo anterior está mal seguramente. Apoyándonos con el ejercicio 7 en el que se calcula la mediana, la mitad de los descuentos realizados estarán en el intervalo 

$$ \left [ 5, 10.85786  \right ] $$

#NOTA2: 

Otra posibilidad es que la mitad de los descuentos realizados estén en el intervalo 

$$ \left [ \mu \pm \sigma \right ] $$

$$ \left [ \frac{35}{3} \pm \sqrt{\frac{200}{9}} \right ] = $$

$$ \left [ 6.952621, 16.38071 \right ] $$
